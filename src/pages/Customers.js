...
// ✅ FIX: Move CustomerModal outside and memoize it
          ...
          {editMode ? (
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border border-white/10 bg-white/5 
                    focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all text-white"
                />
              </div>
              ...
              <div className="flex items-center gap-3 pt-4">
                ...
                <button
                  type="button"
                  onClick={() => onSave(selectedCustomer._id, editFormData)}
                  disabled={loading.update}
                  className="flex-1 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 
...
const Customers = () => {
  ...
  const handleUpdateCustomer = async (userId, formData) => {
  ...
  // ✅ FIX: functional updater ensures no stale state
  const handleEditFormChange = (e) => {
    const { name, value } = e.target;
    setEditFormData(prev => ({ ...prev, [name]: value }));
  };

  const getUniqueFilters = () => ['All', ...new Set(customers.map(c => c.status).filter(Boolean))];

  const filteredAndSortedCustomers = () => {
    let filtered = filter === 'All' ? customers : customers.filter(c => c.status === filter);

    if (searchTerm) {
      filtered = filtered.filter(c =>
        (c.name || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (c.mobileNumber?.toString() || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (c.hostelId?.name || '').toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    return filtered.sort((a, b) => {
      ...
      else if (sortConfig.key === 'mobileNumber') { aVal = a.mobileNumber?.toString() || ''; bVal = b.mobileNumber?.toString() || ''; }
  ...
  const getPaginatedData = () => {
    const filtered = filteredAndSortedCustomers();
  ...
  const { data: paginatedCustomers, totalItems, totalPages } = getPaginatedData();
  ...
  const toggleSelectAll = () => {
    setSelectedCustomers(
      selectedCustomers.length === paginatedCustomers.length ? [] : paginatedCustomers.map(c => c._id)
    );
  };
  ...
  const ActionBar = () => (
    <div className="flex flex-wrap items-center gap-3 mb-6">
      ...
      <div className="flex items-center gap-2 ml-auto">
        ...
        <button onClick={exportToCSV} className="p-2.5 rounded-xl bg-white/10 text-gray-300 hover:bg-white/20 transition-all" title="Export to CSV">
          <Download size={18} />
        </button>
        ...
        <table className="w-full">
          ...
          <tbody>
            {paginatedCustomers.map((customer) => (
              <tr key={customer._id} className="border-b border-white/5 hover:bg-white/10 transition-all duration-300 group">
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => viewCustomer(customer)}
                      className="p-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-500 text-white hover:shadow-lg transition-all opacity-0 group-hover:opacity-100" title="View details">
                      <Eye size={14} />
                    </button>
                    <button onClick={() => openEditModal(customer)}
                      className="p-2 rounded-lg bg-gradient-to-r from-yellow-500 to-orange-500 text-white hover:shadow-lg transition-all opacity-0 group-hover:opacity-100" title="Edit customer">
                      <Edit size={14} />
                    </button>
                    <button onClick={() => handleDelete(customer._id)} disabled={loading.delete}
                      className="p-2 rounded-lg bg-gradient-to-r from-red-500 to-rose-500 text-white hover:shadow-lg transition-all opacity-0 group-hover:opacity-100" title="Delete">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
      <PageHeader
        ...
        subtitle={`${customers.length} total customers • ${customers.filter(c => c.isVerified).length} verified`}
      />
      <StatsCard />
      <ActionBar />
      <FilterBar />
