...
import {
  ...
  Download, RefreshCw, CheckCircle as CheckCircleIcon,
  SortAsc, SortDesc, Search, TrendingUp,
  BadgeCheck, Clock, Tag, ChevronLeft,
...
// ─── ALL sub-components defined outside Enquiries ─────────────────────────────
...
const FilterBar = ({ filter, setFilter, getUniqueFilters }) => {
...
const ActionBar = ({
  ...
  selectedTickets, handleBulkDelete,
  exportToCSV, fetchTickets, loadingFetch,
...
const TableView = ({
  paginatedTickets, selectedTickets, toggleSelectAll, toggleSelect,
  handleSort, sortConfig, viewTicket, handleUpdateStatus, handleDelete,
  loadingUpdate, loadingDelete, totalItems
}) => (
  <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 shadow-lg overflow-hidden">
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-white/10 border-b border-white/10">
            ...
            {[
              { key: 'title',     label: 'Title',    icon: <Tag size={14} /> },
              { key: 'userId',    label: 'Customer', icon: <User size={14} /> },
              { key: 'message',   label: 'Message',  icon: <MessageCircle size={14} /> },
        ...
        <tbody>
          {paginatedTickets.map((ticket) => (
            <tr key={ticket._id} className="border-b border-white/5 hover:bg-white/10 transition-all duration-300 group">
              ...
              <td className="px-4 py-4">
                <div className="flex items-center justify-end gap-2">
                  ...
                  {ticket.status !== 'resolved' && (
                    <button onClick={() => handleUpdateStatus(ticket._id, 'resolved')} disabled={loadingUpdate}
                      className="p-2 rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:shadow-lg transition-all"
                      title="Mark as Resolved">
                      <CheckCircleIcon size={14} />
                    </button>
                  )}
                  <button onClick={() => handleDelete(ticket._id)} disabled={loadingDelete}
    ...
    <div className="px-4 py-3 bg-white/10 border-t border-white/10 flex items-center justify-between text-sm">
      <span className="text-gray-400">Showing {paginatedTickets.length} of {totalItems} tickets</span>
      <span className="text-gray-400">{selectedTickets.length} selected</span>
    </div>
  </div>
);

const TicketModal = ({ selectedTicket, setShowModal, handleUpdateStatus, handleDelete, loadingUpdate, loadingDelete }) => {
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-white/10">
            {selectedTicket.status !== 'resolved' && (
              <button
                onClick={() => { handleUpdateStatus(selectedTicket._id, 'resolved'); setShowModal(false); }}
              ...
              >
                <CheckCircleIcon size={18} />Mark as Resolved
              </button>
            )}
            <button
              onClick={() => { handleDelete(selectedTicket._id); setShowModal(false); }}
...
const Enquiries = () => {
  ...
  const handleUpdateStatus = useCallback(async (ticketId, newStatus) => {
    const result = await Swal.fire({
      ...
      icon: 'question', showCancelButton: true, background: '#0f172a', color: '#fff',
      customClass: {
    ...
    try {
      ...
      fetchTickets();
      setSelectedTicket(prev => prev && prev._id === ticketId ? { ...prev, status: newStatus } : prev);
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = useCallback(async (ticketId) => {
    ...
    try {
      ...
      fetchTickets();
      setSelectedTickets(prev => prev.filter(sid => sid !== ticketId));
      setSelectedTicket(prev => {
        if (prev && prev._id === ticketId) { setShowModal(false); return null; }
        return prev;
      });
    } catch (error) {
      console.error(error);
  ...
  const handleBulkDelete = useCallback(async () => {
    if (selectedTickets.length === 0) { showAlert('warning', 'No selection', 'Please select tickets to delete'); return; }
    const result = await Swal.fire({
      title: 'Delete Selected?', text: `You are about to delete ${selectedTickets.length} tickets`,
    ...
    try {
      setLoading(prev => ({ ...prev, delete: true }));
      for (const id of selectedTickets) await axios.delete(`${API}/deleteticket/${id}`);
  ...
  const viewTicket = useCallback((ticket) => {
    setSelectedTicket(ticket);
    setShowModal(true);
  }, []);

  const getUniqueFilters = useCallback(() =>
    ['All', ...new Set(tickets.map(t => t.status).filter(Boolean))],
  [tickets]);
  ...
  const filteredAndSortedTickets = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      let aVal = sortConfig.key === 'userId' ? a.userId?.name || '' : sortConfig.key === 'createdAt' ? new Date(a.createdAt).getTime() : a[sortConfig.key];
      let bVal = sortConfig.key === 'userId' ? b.userId?.name || '' : sortConfig.key === 'createdAt' ? new Date(b.createdAt).getTime() : b[sortConfig.key];
      if (aVal < bVal) return sortConfig.direction === 'asc' ? -1 : 1;
      if (aVal > bVal) return sortConfig.direction === 'asc' ? 1 : -1;
      return 0;
    });
  }, [tickets, filter, searchTerm, sortConfig]);

  const { paginatedTickets, totalItems, totalPages } = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return {
      paginatedTickets: filteredAndSortedTickets.slice(start, start + itemsPerPage),
      totalItems: filteredAndSortedTickets.length,
      totalPages: Math.ceil(filteredAndSortedTickets.length / itemsPerPage)
    };
  }, [filteredAndSortedTickets, currentPage, itemsPerPage]);

  const toggleSelectAll = useCallback(() => {
    setSelectedTickets(selectedTickets.length === paginatedTickets.length ? [] : paginatedTickets.map(t => t._id));
  }, [selectedTickets.length, paginatedTickets]);

  const toggleSelect = useCallback((id) => {
    setSelectedTickets(prev => prev.includes(id) ? prev.filter(sid => sid !== id) : [...prev, id]);
  }, []);

  const exportToCSV = useCallback(() => {
    const headers = ['Title', 'Customer', 'Message', 'Status', 'Created Date', 'Last Updated'];
    const csvData = filteredAndSortedTickets.map(t => [
      t.title || 'N/A', t.userId?.name || 'N/A', t.message || 'N/A', t.status || 'N/A',
